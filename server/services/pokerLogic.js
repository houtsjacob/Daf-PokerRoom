...
class TexasHoldemUtils {
        ...
        global.timers[entryCode] = new Timer(
            ...
            (finished, currentTime) => {
                if (finished) {
                    ...
                    this.updateTurn(gameState);
                    this.startBettingTimer(entryCode, emitGameState, emitTimer);
    ...
    static startBettingTimer(entryCode, emitGameState, emitTimer) {
        console.log("made it here");
        delete global.timers[entryCode];
        global.timers[entryCode] = new Timer(
            entryCode,
            15,
            (finished, currentTimer) => {
                if (finished) {
                    this.processAction(
                        entryCode,
                        {
